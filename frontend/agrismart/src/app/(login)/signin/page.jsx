...
export default function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Attempting login with:", formData.phoneNumber);
      
      const response = await authAPI.login(formData.phoneNumber, formData.password);
      ...
      if (response.role === "admin") {
        router.push("/admin1");
      } else if (!response.profileCompleted) {
